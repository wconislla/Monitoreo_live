# refresh_data_lucia.ps1 - Recoleccion automatica de estado de backups SANTA LUCIA
# Oracle (RMAN + EXPDP) y SQL Server (Full + Diferencial)
# Genera backup_status_lucia.json para el dashboard de monitoreo

$ErrorActionPreference = "Stop"
$scriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$jsonPath = Join-Path $scriptDir "backup_status_lucia.json"
$dllPath = Join-Path $scriptDir "lib\Oracle.ManagedDataAccess.dll"

# --- Configuracion ---
$oraHost = "192.168.9.17"
$oraPort = "1521"
$oraService = "PROD"
$oraUser = "monitororcl"
$oraPass = "kbZ35AC4OCelZV5Y"

$sqlServer = "192.168.9.12"
$sqlDB = "msdb"
$sqlUser = "monitor"
$sqlPass = "kbZ35AC4OCelZV5Y"
...
if ($rmanFullData.Count -gt 0) {
    $lastStart = $rmanFullData[0]["FECHA"]
    $rmanFilesQuery = "SELECT p.HANDLE as ARCHIVO, p.DEVICE_TYPE as TIPO, TO_CHAR(p.START_TIME,'HH24:MI') as INICIO, TO_CHAR(p.COMPLETION_TIME,'HH24:MI') as FIN, ROUND(p.BYTES/1024/1024, 2) as TAMANO_MB, p.STATUS FROM V`$BACKUP_PIECE p WHERE p.START_TIME >= TO_DATE('$lastStart','YYYY-MM-DD') AND p.START_TIME < TO_DATE('$lastStart','YYYY-MM-DD')+1 AND p.HANDLE LIKE '%bkp_0%' ORDER BY p.START_TIME"
...
if ($rmanArchData.Count -gt 0) {
    $lastArchDate = $rmanArchData[0]["FECHA"]
    $archFilesQuery = "SELECT p.HANDLE as ARCHIVO, TO_CHAR(p.START_TIME,'HH24:MI') as INICIO, TO_CHAR(p.COMPLETION_TIME,'HH24:MI') as FIN, ROUND(p.BYTES/1024/1024, 2) as TAMANO_MB, p.STATUS FROM V`$BACKUP_PIECE p WHERE p.START_TIME >= TO_DATE('$lastArchDate','YYYY-MM-DD') AND p.START_TIME < TO_DATE('$lastArchDate','YYYY-MM-DD')+1 AND p.HANDLE LIKE '%bkp_arc%' ORDER BY p.START_TIME"
...
$expdpDir = "EXPDP"
try {
    $dirData = Get-OracleData -Query "SELECT DIRECTORY_NAME, DIRECTORY_PATH FROM ALL_DIRECTORIES WHERE DIRECTORY_NAME IN ('EXPDP','CDATA','DATA_PUMP_DIR') ORDER BY CASE DIRECTORY_NAME WHEN 'CDATA' THEN 1 WHEN 'EXPDP' THEN 2 ELSE 3 END"
...
# RMAN Full - ultimo COMPLETED del dia (preferir COMPLETED sobre FAILED/RUNNING)
$rmanFullObj = @{ status = "NOT_FOUND"; inicio = ""; fin = ""; minutos = 0; tamano = "0G"; fecha = "" }
if ($rmanFullData.Count -gt 0) {
    $hoyStr = Get-Date -Format "yyyy-MM-dd"
    # Buscar primero un COMPLETED de hoy
    $last = $rmanFullData | Where-Object { $_['FECHA'] -eq $hoyStr -and $_['STATUS'] -eq 'COMPLETED' } | Select-Object -First 1
    # Si no hay COMPLETED hoy, buscar RUNNING de hoy
    if (-not $last) { $last = $rmanFullData | Where-Object { $_['FECHA'] -eq $hoyStr -and $_['STATUS'] -like 'RUNNING*' } | Select-Object -First 1 }
    # Si no hay nada de hoy, tomar el mas reciente (cualquier estado)
    if (-not $last) { $last = $rmanFullData[0] }
    $rmanFullObj = @{
...
# RMAN Arch - verificar 3 ejecuciones diarias (07:00, 14:00, 20:00)
$rmanArchObj = @{ status = "NOT_FOUND"; inicio = ""; fin = ""; minutos = 0; tamano = "0G"; fecha = ""; ejecutados = 0; total_diario = 3 }
if ($rmanArchData.Count -gt 0) {
    $hoyFecha = Get-Date -Format "yyyy-MM-dd"
    $archHoy = @($rmanArchData | Where-Object { $_['FECHA'] -eq $hoyFecha })
    $archCount = $archHoy.Count
    $totalGbHoy = ($archHoy | ForEach-Object { [double]$_['TAMANO_GB'] } | Measure-Object -Sum).Sum
    $last = $rmanArchData[0]
    if ($archCount -ge 3) {
        $archStatus = "COMPLETED"
    } elseif ($archCount -gt 0) {
        $archStatus = "COMPLETED"  # parcial - se marca en el dashboard con ejecutados/total
    } else {
        $archStatus = "NOT_FOUND"
    }
    $rmanArchObj = @{
        status       = $archStatus
        inicio       = [string]$last['INICIO']
        fin          = [string]$last['FIN']
        minutos      = [double]$last['MINUTOS']
        tamano       = "$([math]::Round($totalGbHoy, 2))G"
        fecha        = [string]$last['FECHA']
        ejecutados   = $archCount
        total_diario = 3
    }
}
...
# Full semanal = registros con tipo 'D', Diario = tipo 'I'
foreach ($f in $fechasOrdenadas) {
    $regsDelDia = $sqlByDate[$f]
    $regsDiff = @($regsDelDia | Where-Object { $_.tipo -eq 'I' })
    $regsFull = @($regsDelDia | Where-Object { $_.tipo -eq 'D' })
    if (-not $ultimoDiario -and $regsDiff.Count -gt 0) {
        $ultimoDiario = $regsDiff
        $ultimoDiarioFecha = $f
    }
    if (-not $ultimoSabado -and $regsFull.Count -gt 0) {
        $ultimoSabado = $regsFull
        $ultimoSabadoFecha = $f
    }
...
foreach ($f in $fechasOrdenadas) {
    ...
    $fullEntry = @{ status = "NOT_FOUND"; bases = 0; tamano_gb = 0; comprimido_gb = 0; inicio = ""; fin = ""; nota = "Solo sabados" }
    $diffEntry = @{ status = "NOT_FOUND"; bases = 0; tamano_gb = 0; comprimido_gb = 0; inicio = ""; fin = "" }
    
    if ($itemsFull.Count -gt 0) {
        $totalTam = ($itemsFull | ForEach-Object { $_.tamano_gb } | Measure-Object -Sum).Sum
        $totalComp = ($itemsFull | ForEach-Object { $_.comprimido_gb } | Measure-Object -Sum).Sum
        $sorted = @($itemsFull | Sort-Object { $_.inicio })
        $inicioMin = $sorted[0].inicio
        $finMax = ($itemsFull | Sort-Object { $_.fin } -Descending | Select-Object -First 1).fin
        $fullEntry = @{
            status = "COMPLETED"; tipo = "D"; bases = $itemsFull.Count
            tamano_gb = [math]::Round($totalTam, 2); comprimido_gb = [math]::Round($totalComp, 2)
            inicio = $inicioMin; fin = $finMax; nota = "Full semanal $($itemsFull.Count) bases"
        }
    }
    if ($itemsDiff.Count -gt 0) {
        $totalTam = ($itemsDiff | ForEach-Object { $_.tamano_gb } | Measure-Object -Sum).Sum
        $totalComp = ($itemsDiff | ForEach-Object { $_.comprimido_gb } | Measure-Object -Sum).Sum
        $sorted = @($itemsDiff | Sort-Object { $_.inicio })
        $inicioMin = $sorted[0].inicio
        $finMax = ($itemsDiff | Sort-Object { $_.fin } -Descending | Select-Object -First 1).fin
        $diffEntry = @{
            status = "COMPLETED"; tipo = "I"; bases = $itemsDiff.Count
            tamano_gb = [math]::Round($totalTam, 2); comprimido_gb = [math]::Round($totalComp, 2)
...
$resultado = [ordered]@{
    ...
    mssql = [ordered]@{
        server    = "Santa Lucia SQL"
        host      = $sqlServer
        database  = "IMPULSA"
        instancia = "BD SQL Santa Lucia"
...
Write-Host "[$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')] backup_status_lucia.json actualizado correctamente"
